...
model{
  ...
  for(n in 1:N){
    ...
    mu[n] = lapse[S_id[n]] + (1 - 2 * lapse[S_id[n]]) * (1-exp(-10^(beta[S_id[n]]*(X[n,2]-alpha[S_id[n]]))));
...
generated quantities{
  ...
  for (n in 1:N){
    ...
    log_lik[n] = bernoulli_lpmf(Y[n] | lapse[S_id[n]] + (1 - 2 * lapse[S_id[n]]) * (1-exp(-10^(beta[S_id[n]]*(X[n,2]-alpha[S_id[n]]))))) + lognormal_lpdf(RT[n] - ndt[S_id[n]] | intercept[S_id[n]] + beta_rt[S_id[n]]*(lapse[S_id[n]] + (1 - 2 * lapse[S_id[n]]) * (1-exp(-10^(beta[S_id[n]]*(X[n,2]-alpha[S_id[n]])))))*(1-(lapse[S_id[n]] + (1 - 2 * lapse[S_id[n]]) * (1-exp(-10^(beta[S_id[n]]*(X[n,2]-alpha[S_id[n]])))))), sigma[S_id[n]]);
